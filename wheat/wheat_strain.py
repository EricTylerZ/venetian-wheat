...
class WheatStrain:
    def __init__(self, task, strain_id, coder_model):
        ...
        self.strain_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "strains", f"seed_{self.strain_id}")
    ...
    def generate_code(self, rescue_code=None, rescue_error=None):
        ...
        for attempt in range(retries):
            try:
                ...
                print(f"Seed {self.strain_id}: Coder API response received")
                ...
                self.progress["code_file"] = os.path.join(log_dir, f"seed_{self.strain_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py")
                with open(self.progress["code_file"], "w", encoding="utf-8") as f:
                    f.write(code)
                print(f"Seed {self.strain_id}: Code file saved to {self.progress['code_file']} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                self.save_progress()
                break
                ...
                if attempt == retries - 1:
                    ...
                    print(f"Seed {self.strain_id}: Retrying in {wait_time:.2f}s due to: {str(e)[:100]}")
                    time.sleep(wait_time)
            except Exception as e:
                print(f"Seed {self.strain_id}: Unexpected error in generate_code - {str(e)}")
                self.progress["status"] = "Barren"
    ...
    def grow_and_reap(self):
        if "API error" in self.task or not self.code:
            ...
            self.save_progress()
            return f"[{self.progress['timestamp']}] [seed_{self.strain_id}] [{self.task}] [Barren] [No code generated]"
        ...
        if "OK" in self.progress["test_result"]:
            ...
            log_entry = f"[{timestamp}] [seed_{self.strain_id}] [{self.task}] [Fruitful] [OK] [Output: {output[:50]}...] [Code: {self.progress['code_file']}]"
        else:
            ...
            if self.retry_count < 2:
                ...
                self.progress["status"] = "Barren"
                log_entry = f"[{timestamp}] [seed_{self.strain_id}] [{self.task}] [Barren] [FAILED] [{error_msg[-100:]}]"
