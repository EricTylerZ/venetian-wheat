...
class WheatSeed:
    ...
    def save_progress(self):
        ...
        try:
            ...
            if c.rowcount == 0:
                max_id_row = c.execute("SELECT MAX(id) FROM runs").fetchone()
                ...
                c.execute("INSERT INTO seeds (run_id, seed_id, task, status, output, code_file, test_result) VALUES (?, ?, ?, ?, ?, ?, ?)",
                          (run_id, self.seed_id, self.task, self.progress["status"], json.dumps(self.progress["output"]), self.progress["code_file"], self.progress["test_result"]))
            conn.commit()
        finally:
            conn.close()
        os.makedirs(self.seed_dir, exist_ok=True)
